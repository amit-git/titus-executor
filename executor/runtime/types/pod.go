...
func NewPodContainer(pod *corev1.Pod, ipv4Address *string) (*PodContainer, error) {
	...
	if err := c.extractContainerInfoFromPod(); err != nil {
		return nil, err
	}
...
func (c *PodContainer) extractContainerInfoFromPod() error {
	str, ok := c.pod.GetAnnotations()["containerInfo"]
	if !ok {
	...
	if err != nil {
		return errors.Wrap(err, "unable to base64 decode containerInfo annotation")
	}
